...
typedef void (*BeforeWDRebootCallback)();
...
class Watchdog {
	...
	//BeforeWDRebootCallback ptrBeforeRebootCallback;
...
void Watchdog::timerEvent() {
	...
	if (wdt_counter < eventsReset) {
		...
		//// callback first
		//if (ptrBeforeRebootCallback) ptrBeforeRebootCallback();
		// go for reset
		wdt_hw_reboot();
	}
}
